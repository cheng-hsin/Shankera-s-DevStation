...
1. 訪問以下站點並下載 OpenSSH。一般在Windows 2012R2上，選擇【OpenSSH-Win64.zip】進行下載。
...
6. 運行 [Powershell] 並移動到您所在的 OpenSSH 文件夾。接下來，運行命令“.\install-sshd.ps1”來安裝 sshd 服務，如下所示。如果顯示成功消息，則安裝成功。(最好自己key，不要複製)
    
    ![](https://hackmd.io/_uploads/HJ3JXE2pn.png)

7. 接下來，生成 SSH 主機密鑰。運行命令“.\ssh-keygen.exe -A”，如下所示。(最好自己key，不要複製)

    ![](https://hackmd.io/_uploads/H1TZmE2p2.png)

8. 安裝sshd就可以了。接下來，打開【服務】並啟動 sshd。並且將[啟動類型]更改為[自動]，如下所示。
