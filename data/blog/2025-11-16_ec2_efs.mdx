---
title: '在 EC2 掛載 EFS 完整指南'
date: '2025-11-16'
lastmod: '2025-11-16'
tags: ['aws', 'ec2', 'efs', 'storage']
draft: false
summary: '在 EC2 環境下，如何正確掛載 EFS 文件系統。'
layout: PostSimple
...
3. 點選該 SG → **Inbound rules** → **Edit inbound rules**
4. 新增以下規則：
...
2. ✅ **建立掛載目錄**：`mkdir -p /efs/mnt/efs_staging`
3. ✅ **執行掛載指令**：`sudo mount -t efs -o tls <EFS-ID>:/ <掛載點>`
...
- [AWS EFS 官方文件](https://docs.aws.amazon.com/efs/)
- [EFS Mount Helper 安裝指南](https://docs.aws.amazon.com/efs/latest/ug/mounting-fs-mount-helper.html)
- [Security Group 最佳實踐](https://docs.aws.amazon.com/vpc/latest/userguide/VPC_SecurityGroups.html)
